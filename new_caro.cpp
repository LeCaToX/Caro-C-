...
public:
    ...
    int calcNextMove(int depth) {
        ...
        if (bestMove.first != -1) {
            ...
            bestMove = minimaxSearch(depth,Board(board),true,-1.0,getWinScore());
            if (bestMove[1] == -1) move = -1;
            else {
                move[0] = bestMove[1];
                move[1] = bestMove[2];
            }
    ...
    */
    pair<int,int> minimaxSearch(int depth, Board tmpBoard, bool Max, double alpha, double beta) {
        ...
        //If there is no such available move, consider this node like the base case
        ...
        if (Max) {
            ...
            for (int i = 0; i < allMove.size(); i++) {
                ...
                int tmpMove = minimaxSearch(depth-1,tmpBoard,false,alpha,beta);
                tmpBoard.removeMove(allMove[i][1],allMove[i][0]);
                //Update the alpha (maximum score)
                //If the score of current node is lower than alpha, the whole subtree of this node will be ignored.
                if (tmpMove[0] > alpha) alpha = tmpMove[0];
                //Pruning with the beta (minimum score)
                //We need the score which is lower than current beta
                //-> if the score is higher or equal to current beta, ignore all node and subtree and return the last move.
            ...
            for (int i = 0; i < allMove.size(); i++) {
                ...
                //Gen new minimax tree from current node
                //Search for the minimum score for each subtree of each nodes
                int tmpMove = minimaxSearch(depth-1,tmpBoard,true,alpha,beta)
                tmpBoard.removeMove(allMove[i][1],allMove[i][0]);
                //Update the beta (minimum score)
                //If the score of current node is hgiher than beta, the whole subtree of this node will be ignored.
                if (tmpMove[0] < beta) beta = tmpMove[0];
                //Pruning with alpha
                //We need the score which is hight than current alpha
                //-> if the score is lower or equal to current beta, ignore all node and subtree and return the last move.
    ...
    static int evaluateHorizontal(vector<vector<int>> board, bool forSecond, bool curTurn) {
    ...
    static int evaluateVertical(vector<vector<int>> board, bool forSecond, bool curTurn) {
        // 0: consecutive count, 1: block count, 2: score
        int evalutions[3] = {0,2,0};
        for (int j = 0; j < board.size(); j++) {
            for (int i = 0; i < board.size(); i++) {
                evaluateDirections(board,i,j,forSecond,playerTurn,evalutions);
            }
            evaluateDirectionAfterOnePass(evalutions,forSecond,curTurn);
        }
        return evalutions[2];
    ...
    static int evaluateDiagonal(vector<vector<int>> board, bool forSecond, bool curTurn) {
        // 0: consecutive count, 1: block count, 2: score
        int evalutions[3] = {0,2,0};
        ...
        for (int k = 0; k <= 2 * (board.size() - 1); k++) {
            ...
            for (int i = iStart; i <= iEnd; ++i) {
                evaluateDirections(board, i, k - i, forSecond, curTurn, evaluations);
            }
            evaluateDirectionsAfterOnePass(evaluations, forSecond, curTurn);
        }

        // From top-left to bottom-right diagonally
        for (int k = 1 - static_cast<int>(board.size()); k < static_cast<int>(board.size()); k++) {
            int iStart = max(0, k);
            int iEnd = min(static_cast<int>(board.size() + k - 1), static_cast<int>(board.size()) - 1);
            for (int i = iStart; i <= iEnd; ++i) {
                evaluateDirections(board, i, i - k, forSecond, curTurn, evaluations);
            }
            evaluateDirectionsAfterOnePass(evaluations, forSecond, curTurn);
        }
    ...
    static void evaluateDirections(vector<vector<int>> booard, int i, int j, bool isBot, bool botTurn, int eval[3]) {
    ...
    static void evaluateDirectionsAfterOnePass(int eval[3], bool isBot, bool playersTurn) {
        // End of row, check if there were any consecutive stones before reaching the right border
        if (eval[0] > 0) {
    ...
    static int getConsecutiveSetScore(int cnt, int blocks, bool curTurn) {
        ...
        // If both sides of a set are blocked and this set has less than 5 consecutive stones, it is worthless, return 0 points.
        ...
        switch (count) {
            ...
            case 4:
                ...
                if (curTurn) return winGuarantee;
                else {
                    // Opponent's turn
